...
function InvoiceCreate ({}) {
    ...
    const [formData, setFormData] = React.useState({vat: 7.5, service_charge: 10, account_name: "BLOOM DIGITAL MEDIA LTD", account_number: 5600579630, bank_name: "Fidelity Bank PLC"})
    const [lineItems, setLineItems] = React.useState([{item: "", desc: "", qty: "", price: ""}])
    ...
    return <>
        <div className="m-5">
            ...
            <div className="my-4 flex justify-start">
                <button className="uppercase shadow px-2 text-blue-700 hover:bg-slate-100" onClick={ () => navigate('/')}> Invoices</button>
            </div>
        ...
        <div className="m-5">
            <table className="my-3 ">
                ...
                <tbody>
                    {
                        lineItems?.length > 0 && lineItems?.map( (row, index) => (
                            <tr key={index}>
                                <td>{index + 1}</td>
            ...
            <div className="mt-8">
            <button onClick={createInvoice} className="bg-orange-200 hover:bg-orange-300  font-medium rounded text-sm px-5 py-0.5 mr-2 mb-2">
